...
def search_all_stores(keyword: str, user_id=None) -> tuple:
    ...
    # TTL = 24 giờ — đồng bộ với get_data_from_db(ttl_hours=24)
    ...
    if cached:
        ...
        if last_updated:
            ...
            if time_diff < timedelta(hours=24):
                print(f"⚡ [Fast Load] Trả về {len(cached)} kết quả từ DB cho '{normalized_kw}' (cache còn {24 - time_diff.total_seconds()//3600:.0f}h)")
                return cached, True
