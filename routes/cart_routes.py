...
def send_order_confirmation_async(order_id, fullname, email, total_str, payment_method):
    """
    Tác vụ chạy ngầm để gửi email xác nhận và tích hợp API/Webhook bên thứ ba.
    Giúp người dùng không phải chờ các tác vụ mạng/I/O đồng bộ.
    """
    print(f"🚀 [Background Thread] Bắt đầu xử lý đơn hàng #{order_id} cho khách hàng {fullname} ({email}).")
    try:
        # 1. Giả lập gửi email xác nhận qua dịch vụ mail (SMTP/Sendgrid)
        print(f"📧 [Email Service] Đang gửi email xác nhận đến {email}...")
        time.sleep(2.0)  # Giả lập độ trễ mạng khi kết nối SMTP server
        print(f"📧 [Email Service] ✅ Gửi email thành công cho đơn hàng #{order_id}!")

        # 2. Giả lập gọi API cổng thanh toán hoặc đồng bộ CRM/Webhook của bên thứ ba
        print(f"🔗 [Third-party API] Đang kết nối cổng thanh toán / đồng bộ đơn hàng #{order_id} lên hệ thống CRM...")
        time.sleep(1.5)  # Giả lập độ trễ API call
        print(f"🔗 [Third-party API] ✅ Tích hợp đơn hàng #{order_id} thành công!")

        # 3. Ghi log hoàn thành
        print(f"📝 [System Log] Hoàn tất toàn bộ tác vụ phụ cho đơn hàng #{order_id} một cách trơn tru.")
    except Exception as e:
        print(f"❌ [System Log] Lỗi khi xử lý tác vụ phụ cho đơn hàng #{order_id}: {str(e)}")
...
def checkout():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # Tách tác vụ phụ (gửi mail + gọi API bên thứ ba) chạy bất đồng bộ qua Background Thread
            thread = threading.Thread(
                target=send_order_confirmation_async,
                args=(last_order_id, fullname, email, total_str, payment_method)
            )
            thread.daemon = True
            thread.start()
